...
<body>
<%
DessertDAOImpl ddao = new DessertDAOImpl();
Menu.liste_dessert=ddao.allDesserts();
%>
	<div class="row mx-2 my-2">
		<div class="col-md-8 my-5 py-5">
			...
			<div class="container">
				...
				<%for(DessModel d:Menu.liste_dessert){ %>
				    <div class="col">
				      	<div class="animate__animated animate__bounceInLeft h-100">
							<div class="mycard myshadow card-bg-red h-100 mx-2 p-2">
							  <img class="img-fluid p-1 my-3" style="border-radius: 20px" width="225px" height="225" src="../images/<%=d.getImgDess() %>" alt="pic"/>
							  <h5 class="text-center text-white fontCentury my-3 py-1"><%=d.getNomDess() %></h5>
							  <h6 class="text-center text-white fontCentury py-1">Prix: <%=d.getPrixDess() %> DH</h6>
							  <div class="d-grid gap-1">
							 <a class="btn btn-danger" href='${pageContext.request.contextPath}/AjouterDessertPanier?id=<%=d.getIdDess()%>&nom=<%=d.getNomDess()%>&prix=<%=d.getPrixDess()%>&img=<%=d.getImgDess()%>'>Ajouter</a>
							  </div>
							</div>
						</div>
				    </div>
